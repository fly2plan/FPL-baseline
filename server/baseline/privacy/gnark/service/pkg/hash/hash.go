...
const (
	ACKPadding = "29079322780549167436035941967647618293475273399992955110144160538034176"
)

const ACK = 656775

var hFunc = gHash.MIMC_BN254.New()
...
func HashFPL(parsedForm fpl.ParsedFPLForm) []byte {
...
func HashACK(FPLHash []byte) []byte {
	ACK := big.NewInt(ACK)
	padding, _ := big.NewInt(0).SetString(ACKPadding, 10)
	hFunc.Write(FPLHash)
	ACK.Mul(ACK, padding)
	hFunc.Write(ACK.Bytes())
	hash := hFunc.Sum(nil)
	hFunc.Reset()
	return hash
}
